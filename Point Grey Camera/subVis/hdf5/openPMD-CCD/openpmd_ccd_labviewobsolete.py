...
def open_write(directory_path, ccd_name, scan_number=None, ccd_model=None,
    ...
    scan[ccd_name] = CCD(
        ...
        ccd_resolution, ccd_roi, ccd_exposure_time, False)
    # FIXME: try-catch and return status "cluster" (tuple)

import numpy as np
import ctypes
import matplotlib.pyplot as plt
#def add(ccd_name, image_number, image_data, image_path=None):
#def add(ccd_name, image_number, image_data, image_path=None):
def add(im_ptr, lineWidth, height, width):
    """ FIXME: doc strings """
    #assert False, str(im_ptr) + " " + str(lineWidth) + " " + str(height) + " " + str(width)
    UINT16P = ctypes.POINTER(ctypes.c_uint16)
    ptr = ctypes.cast(im_ptr, UINT16P)
    data_np = np.ctypeslib.as_array(ptr, shape=(1, 1))
    x = data_np[0, 0]
	#assert False, "data: " + str(data_np[0,0])
    #return data_np[0,0]
    #assert ccd_name in scan, "[openPMD-CCD] CCD name not known (open_write called?)."
    #image_numpy = np.array(image_data).astype("uint16")
    #scan[ccd_name].add(image_number, image_data=image_numpy, image_path=image_path)
    #print(image_data[0])
    # FIXME: try-catch and return status "cluster" (tuple)
